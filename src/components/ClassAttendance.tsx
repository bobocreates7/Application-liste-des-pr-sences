...
export default function ClassAttendance({ 
  ...
  return (
    <div className="flex flex-col h-screen max-w-md mx-auto bg-gray-50 shadow-xl overflow-hidden relative">
      {/* Header */}
      <header className="bg-white border-b border-gray-200 p-3 flex items-center justify-between sticky top-0 z-20">
        <button 
          onClick={onBack}
      ...
      <main className="flex-1 overflow-y-auto pb-32">
        <div className="p-3 bg-gray-50 sticky top-0 z-10">
          <div className="relative">
            ...
            <input
              type="text"
              className="block w-full pl-9 pr-3 py-2 border border-gray-200 rounded-xl bg-white text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#1A73E8]/50 focus:border-[#1A73E8] transition-colors text-sm shadow-sm"
        ...
        <div className="divide-y divide-gray-100">
          {filteredStudents.length === 0 ? (
            ...
            filteredStudents.map(student => {
              ...
              return (
                <div key={student.id} className="bg-white p-3 flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    <div className="flex-1 pr-3">
                      <p className="text-base font-medium text-gray-900 uppercase">{student.lastName}</p>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-gray-50 via-gray-50 to-transparent pt-10">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-3 flex flex-col gap-2">
          <div className="flex justify-between items-center px-2">
            <span className="text-sm text-gray-600 font-medium">Total Absents</span>
            <span className={`text-lg font-bold ${absentCount > 0 ? 'text-[#E53935]' : 'text-gray-900'}`}>
              {absentCount}
            </span>
          </div>
